...
from .base import progress_bar, truncated_pareto, \
    ...
    aggregate_counts, make_search_grid, adaptive_xmin_xmax_ks as adaptive_search, \
    gen_surrogate_counts as _surrogate, _work_axis
from .counted_data import hill_estimator as _naive_estimator


def _check_bins_counts(edges, counts, flatten=False, sort=False):
    ...
    if sort and np.any(np.diff(edges, axis=_work_axis) < 0):
...
def _log_likelihood(lefts, widths, counts, xmin, xmax, alpha):
    ...
    This is a correct estimate working for any sort of increasingy ordered bins.
...
def find_xmin_xmax_ks(edges, counts, grid=None, scaling_range=10, max_range=np.inf, req_samples=100,
...
def adaptive_xmin_xmax_ks(*args, **kwargs):
    return adaptive_search(find_xmin_xmax_ks, *args, **kwargs)
